<?php
        ...
        if(isset($_SESSION['UserName'])){
            ...
            if($do == 'Manage'){ // Manage Page
                ...
                $rows = getAllTable("*", "users", "WHERE GroupID != 2", $query, "UserID", "");
      ...
      <?php }elseif($do == 'Insert'){  // Insert Page
                ...
                if($_SERVER['REQUEST_METHOD'] == 'POST'){
                    ...
                    if(empty($formArray)){
                        ...
                        $chick = chickItem("FullName", "users", $user);
                        ...
                        if($chick == 1){
                            ...
                            $stmt = $con->prepare("INSERT INTO 
                                                        users(FullName, Email, `Password`, gender, avatar)
                                                    VALUES(:zuser, :zmail, :zpass, :zgender, :zavatar)");
                            $stmt->execute(array(
                                'zuser'   => $user,
                                'zmail'   => $email,
                                'zpass'   => $hashPass,
                        ...
                        <form class="form-horizontal" action="?do=Update" method="POST" enctype="multipart/form-data">
                            ...
                            <!-- Start UserName -->
                                <div class="mb-2 row">
                                    ...
                                    <div class="col-sm-10 col-md-9">
                                        <input type="text" name="username" value="<?php echo $row['FullName']; ?>" class="form-control" autocomplete="off" required="required">
                                    </div>
                                </div>
                            <!-- End UserName -->
                            <!-- Start Email -->
            ...
            if($_SERVER['REQUEST_METHOD'] == 'POST'){
                ...
                if(empty($formArray)){
                    ...
                    if($count == 1){
                        ...
                        $stmt = $con->prepare("UPDATE users SET FullName = ?, Email = ?, `Password` = ?, gender = ?, avatar = ? WHERE UserID = ?");
                        $stmt->execute(array($user, $email, $pass, $gender, $avatar, $id));
